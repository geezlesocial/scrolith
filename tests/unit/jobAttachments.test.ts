...
test('parses URI-encoded serialized attachments for image previews and open links', () => {
  const payload = {
  ...
  const fullyEncoded = encodeURIComponent(encoded);

  for (const value of [encoded, fullyEncoded]) {
