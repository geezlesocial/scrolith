...
const DEFAULT_CONFIG: ScrolithMatchConfig = {
  enabled: true, minimumScore: 0.2, maxCandidates: 30, dailyInterestLimit: 30, dismissCooldownDays: 30,
...
const ScrolithMatchManagement: React.FC = () => {
  ...
  return <section className="space-y-6">
    ...
    <div className="rounded-lg border border-slate-200 bg-white p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold text-slate-900">Last 30 days</h2><p className="text-xs text-slate-500">Operational counters from the Match event stream.</p></div><button type="button" onClick={() => void save()} disabled={saving || loading} className="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"><Save className="h-4 w-4" />{saving ? 'Saving...' : 'Save controls'}</button></div><div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-5">{[['Shown', analytics?.shown ?? 0], ['Interests', analytics?.interests ?? 0], ['Dismissals', analytics?.dismissals ?? 0], ['Mutual', analytics?.mutualConnections ?? 0], ['Conversion', `${Math.round((analytics?.conversionRate ?? 0) * 100)}%`]].map(([label, value]) => <div key={String(label)} className="rounded-md bg-slate-50 p-3"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-lg font-bold text-slate-900">{value}</p></div>)}</div></div>
  </section>;
};
