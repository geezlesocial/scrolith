...
const CommunityAnalyticsComponent = () => {
    const [data, setData] = useState<CommunityAnalyticsType | null>(null);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        const loadAnalytics = async () => {
            try {
                const payload = await CommunityService.getCommunityAnalytics();
                setData(payload);
                setError(null);
            } catch (err) {
                console.error('Failed to load community analytics:', err);
                setError('Community analytics are unavailable right now.');
                setData(null);
            }
        };
        loadAnalytics();
    ...
    // Safely access the arrays, providing empty arrays as fallbacks if they are undefined
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    ...
                    <div className="h-64 flex items-end justify-between px-4 gap-2">
                        {/* Safely map over engagementTrend */}
                        {engagementTrend.length > 0 ? (
                            engagementTrend.map((val, i) => (
                            ...
                            <div className="w-full text-center text-gray-500 py-8">No engagement trend data available.</div>
                        )}
                    </div>
                </div>
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        ...
                        {topChannels.length > 0 ? (
                            ...
                            <tr>
                                <td colSpan={3} className="text-center py-8 text-gray-500">No top channel data available.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
...
export default CommunityAnalyticsComponent; // Change export name
