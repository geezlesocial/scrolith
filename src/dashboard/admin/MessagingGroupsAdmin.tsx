...
const MessagingGroupsAdmin: React.FC = () => {
  ...
  const [enterpriseSearchQ, setEnterpriseSearchQ] = useState('');
  const [enterpriseSearchCategory, setEnterpriseSearchCategory] = useState('all');
  const [enterpriseHits, setEnterpriseHits] = useState<any[]>([]);
  const [searchTookMs, setSearchTookMs] = useState<number | null>(null);
  const [healthById, setHealthById] = useState<Record<string, number>>({});
  ...
  useEffect(() => {
    ...
    if (tab === 'analytics') {
      void loadOverview();
      void AdminService.getMessagingGroupsAnalytics(14)
        .then((d) => setOverview((prev: any) => ({ ...(prev || {}), analytics29_5: d })))
        .catch(() => null);
    }
  }, [tab, loadOverview, loadGroups, loadAudit, loadJoinRequests, loadTemplates, loadSettings]);

  const runEnterpriseSearch = async () => {
    setLoading(true);
    try {
      const res = await AdminService.searchMessagingGroupsAdmin({
        q: enterpriseSearchQ,
        category: enterpriseSearchCategory,
        limit: 30
      });
      const hits = res?.hits || res?.data?.hits || [];
      setEnterpriseHits(Array.isArray(hits) ? hits : []);
      setSearchTookMs(res?.tookMs ?? res?.data?.tookMs ?? null);
    } catch (e: any) {
      showNotification('error', 'Search', e?.message || 'Search failed');
      setEnterpriseHits([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-4" data-testid="admin-messaging-groups">
      ...
      {tab === 'search' ? (
        <div className="space-y-3" data-testid="mg-admin-enterprise-search">
          <div className="flex flex-wrap gap-2">
            <input
              value={enterpriseSearchQ}
              onChange={(e) => setEnterpriseSearchQ(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && void runEnterpriseSearch()}
              placeholder="Enterprise search across groups, messages, members, audit…"
              className="min-w-[220px] flex-1 rounded-xl border border-slate-200 px-3 py-2 text-sm"
              aria-label="Enterprise messaging groups search"
            />
            <select
              value={enterpriseSearchCategory}
              onChange={(e) => setEnterpriseSearchCategory(e.target.value)}
          ...
          {searchTookMs != null ? (
            <p className="text-xs text-slate-500">Took {searchTookMs}ms · {enterpriseHits.length} hits</p>
          ) : null}
          <ul className="space-y-2">
            {enterpriseHits.map((h) => (
              <li
                key={`${h.type}-${h.id}`}
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  {h.conversationId ? (
                    <button
                      ...
                      onClick={() => void loadDetail(h.conversationId)}
                    >
                      Open group
                    </button>
                  ) : null}
                </div>
                <div className="mt-1 font-medium text-slate-900">{h.title || h.id}</div>
                <div className="text-xs text-slate-600">{h.highlight || h.snippet || '—'}</div>
              </li>
            ))}
            {!enterpriseHits.length ? (
              <li className="text-sm text-slate-500">No results. SECRET non-member data is never returned.</li>
            ) : null}
          </ul>
        </div>
      ) : null}
      ...
      {tab === 'overview' || tab === 'analytics' ? (
        <div className="space-y-4" data-testid="mg-admin-overview">
          ...
          {overview?.analytics29_5?.totals ? (
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              <StatCard
                title="Msgs (window)"
                value={overview.analytics29_5.totals.messages ?? '—'}
                hint={`${overview.analytics29_5.windowDays || 14}d`}
              />
              <StatCard title="Voice notes" value={overview.analytics29_5.totals.voiceNotes ?? '—'} />
              <StatCard
                title="Groups created"
                value={overview.analytics29_5.totals.groupsCreated ?? '—'}
              />
              <StatCard
                title="Attachment refs"
                value={overview.analytics29_5.totals.attachmentReferences ?? '—'}
              />
            </div>
          ) : null}
          {Array.isArray(overview?.analytics29_5?.sampleHealth) ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-4 text-sm">
              <h3 className="font-semibold text-slate-900">Sample health scores</h3>
              <ul className="mt-2 space-y-1">
                {overview.analytics29_5.sampleHealth.map((h: any) => (
                  <li key={h.conversationId} className="flex justify-between text-xs">
                    <span className="truncate">{h.title || h.conversationId}</span>
                ...
                <div className="mt-2 flex flex-wrap gap-2 text-xs">
                  ...
                  {healthById[detail.id] != null ? (
                    ...
                    <button
                      ...
                      onClick={() =>
                        void AdminService.getMessagingGroupHealthAdmin(detail.id).then((h) => {
                          const score = h?.score ?? h?.data?.score;
                          if (score != null) {
                            setHealthById((m) => ({ ...m, [detail.id]: Number(score) }));
                          }
                        })
                      }
                    >
