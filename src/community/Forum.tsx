    ...
    const handleAdminAction = async (action: 'delete' | 'lock' | 'pin', threadId: string) => {
        ...
        try {
            if (action === 'delete') {
                ...
                // Get current thread to check lock status
                const currentThread = threads.find(t => t.id === threadId);
                const currentLocked = currentThread?.status === 'locked';
                const isLocked = await CommunityService.toggleThreadLock(threadId, currentLocked);
                showNotification('info', isLocked ? 'Locked' : 'Unlocked', `Thread ${isLocked ? 'locked' : 'unlocked'}.`);
            } else if (action === 'pin') {
                // Get current thread to check pin status
                const currentThread = threads.find(t => t.id === threadId);
                const currentPinned = currentThread?.isPinned || false;
                const isPinned = await CommunityService.toggleThreadPin(threadId, currentPinned);
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">
                                            {thread.categoryName || thread.category_name || 'General'}
                                        </span>
                                        {(thread.isPinned || thread.is_pinned) && <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded font-bold border border-yellow-200 flex items-center"><Pin className="w-3 h-3 mr-1"/> Pinned</span>}
                                        {(thread.status === 'locked' || thread.isLocked || thread.is_locked) && <span className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded font-bold border border-red-200 flex items-center"><Lock className="w-3 h-3 mr-1"/> Locked</span>}
                                        <span className="text-xs text-gray-400 ml-auto md:ml-0">• Posted by {thread.userName || thread.user_name || 'Anonymous'}</span>
                                    </div>
                            ...
                            {canModerate && (
                                <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <div className="flex gap-1 bg-white border border-gray-200 shadow-sm rounded-lg p-1">
                                        <button onClick={() => handleAdminAction('pin', thread.id)} className={`p-1.5 rounded hover:bg-gray-100 ${(thread.isPinned || thread.is_pinned) ? 'text-yellow-600' : 'text-gray-400'}`} title="Pin"><Pin className="w-4 h-4" /></button>
                                        <button onClick={() => handleAdminAction('lock', thread.id)} className={`p-1.5 rounded hover:bg-gray-100 ${(thread.status === 'locked' || thread.isLocked || thread.is_locked) ? 'text-red-600' : 'text-gray-400'}`} title="Lock"><Lock className="w-4 h-4" /></button>
