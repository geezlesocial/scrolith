...
const SearchResults = () => {
    ...
    useEffect(() => {
        const fetchResults = async () => {
            setLoading(true);
            try {
                const { results } = await SearchService.performSearch(query, mode);
                setResults(results);
            } catch (err) {
                console.error(err);
            } finally {
        ...
        <div className="min-h-screen bg-gray-50 pt-20 pb-12">
            <div className="bg-white border-b border-gray-200 mb-8">
                <div className="max-w-7xl mx-auto px-4 py-8">
                    <SearchInput placeholder="Search for anything..." className="max-w-3xl" searchMode={mode} searchPath="/search" />
                    <div className="mt-6 flex space-x-4 overflow-x-auto pb-2">
                        {['all', 'gig', 'job', 'blog'].map(f => (
                            <button
                                key={f}
                                onClick={() => setFilter(f as 'all' | 'gig' | 'job' | 'blog')}
