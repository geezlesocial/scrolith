...
const AISkillMatchBar = () => {
  ...
  const categories: string[] = Array.isArray(results?.recommendedCategories) ? results.recommendedCategories : [];
  const pricingRange: string = results?.pricingRange || "";
  const gigs: string[] = Array.isArray(results?.suggestedGigs) ? results.suggestedGigs : [];
    ...
    <div className="bg-indigo-900 py-16 text-white relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
        ...
        <div className="bg-white p-2 rounded-xl shadow-2xl flex flex-col sm:flex-row gap-2 max-w-xl mx-auto mb-8">
          ...
          <button
            onClick={handleMatch}
            disabled={isMatching || !input.trim()}
                ...
                <div className="flex flex-wrap gap-2">
                  {categories.map((cat: string, i: number) => (
                    <span
                      ...
                      className="bg-indigo-800/50 hover:bg-indigo-700 px-3 py-1 rounded text-sm cursor-pointer transition"
                    >
                      {cat}
                    </span>
                  ))}
                  {categories.length === 0 && <span className="text-sm text-indigo-200">No suggestions yet.</span>}
                </div>
              </div>

              <div>
                <h4 className="text-xs font-bold text-indigo-200 uppercase mb-2">Suggested Pricing</h4>
                <p className="text-2xl font-bold text-white">{pricingRange || "-"}</p>
                <p className="text-xs text-indigo-300">Estimated range</p>
              </div>

              <div>
                <h4 className="text-xs font-bold text-indigo-200 uppercase mb-2">Recommended Gigs</h4>
                <ul className="space-y-1">
                  {gigs.slice(0, 2).map((gig: string, i: number) => (
                    <li key={i} className="text-sm flex items-center text-indigo-100">
                      <ArrowRight className="w-3 h-3 mr-2" /> {gig}
                    </li>
                  ))}
                  {gigs.length === 0 && <li className="text-sm text-indigo-200">No gig ideas yet.</li>}
                </ul>

                <button
                  onClick={() => navigate("/create-gig")}
                  className="mt-3 text-xs font-bold text-green-400 hover:text-green-300 flex items-center"
                >
