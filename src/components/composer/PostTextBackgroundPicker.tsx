...
export const PostTextBackgroundPicker: React.FC<Props> = ({
  ...
  return (
    <div className={`space-y-2 ${className}`} data-testid="post-text-bg-picker">
      <div className="flex items-center justify-between gap-2">
        <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">
          Background
        </p>
        <p className="text-[11px] text-slate-400">Text color adjusts automatically</p>
      </div>
        ...
        {postTextBackgroundThemes.map((theme: PostTextBackgroundTheme) => {
          ...
          return (
            <button
              ...
              className={`relative h-9 w-9 shrink-0 overflow-hidden rounded-lg border-2 shadow-sm transition focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 disabled:cursor-not-allowed disabled:opacity-50 ${
