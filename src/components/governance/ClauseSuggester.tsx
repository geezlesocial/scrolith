...
const ClauseSuggester: React.FC<Props> = ({ jobDescription, jobType, onAddClause }) => {
  ...
  const generate = async () => {
    ...
    setError(null);
    try {
      // Pass jobDescription for better clause relevance (production accuracy)
      const data = await GovernanceService.suggestContractClauses(jobType, jobDescription);
      setSuggestions(Array.isArray(data) ? data : []);
    } catch (e) {
      console.error("Clause suggestion failed:", e);
    ...
    <div className="border border-indigo-100 bg-indigo-50/50 rounded-xl p-6 mt-6">
      ...
      {suggestions.length === 0 && !loading && !error && (
        <p className="text-sm text-indigo-400 italic">
          Click analyze to detect missing protective clauses for this {jobType} contract.
        </p>
      )}
