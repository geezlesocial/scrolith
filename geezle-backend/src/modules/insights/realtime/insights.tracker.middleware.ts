...
import { trackCareerDailyAction, type CareerDailyActionType } from '../services/careerStreak.service';
...
const inferEventType = (req: Request): string | null => {
  ...
  if (
    method === 'POST' &&
    (
      path.includes('/api/ai/answer') ||
      path.includes('/api/ai/guide') ||
      path.includes('/api/ai/scrolitha-answer') ||
      path.includes('/api/ai/scrolitha-guide')
    )
  ) {
...
const inferCareerDailyActionType = (eventType: string): CareerDailyActionType | null => {
  if (eventType === 'POST_CREATED') return 'post';
  if (eventType === 'COMMENT_CREATED') return 'reply';
  if (eventType === 'PROPOSAL_SUBMITTED') return 'apply';
  if (eventType === 'LEARNING_COMPLETED') return 'learn';
  return null;
};
...
export const insightsActionTrackerMiddleware = (req: Request, res: Response, next: NextFunction) => {
  ...
  const careerActionType = inferCareerDailyActionType(eventType);
  ...
  res.on('finish', () => {
    ...
    void recordInsightEvent({
      ...
      .then(async () => {
        if (!careerActionType) return;
        await trackCareerDailyAction({
          userId,
          actionType: careerActionType,
          sourceType: 'http_request',
          sourceId: `${req.method}:${req.originalUrl}`,
          meta: payload,
          app: req.app
        });
      })
