...
export function validateEnv() {
  ...
  if (missingCritical.length > 0) {
    console.error('⚠️  Missing critical environment variables:');
  ...
  if (missingOptional.length > 0) {
    console.warn('ℹ️  Optional environment variables are not set (some features may be limited):');
  ...
  if (storage === 's3') {
    ...
    if (missing.length > 0) console.warn('⚠️  S3 storage selected but missing:', missing.join(', '));
  }
  if (storage === 'backblaze') {
    const keys = ['B2_BUCKET', 'B2_REGION', 'B2_ACCESS_KEY_ID', 'B2_SECRET_ACCESS_KEY'];
    const missing = keys.filter(k => !process.env[k] || process.env[k]!.trim() === '');
    if (missing.length > 0) console.warn('⚠️  Backblaze B2 selected but missing:', missing.join(', '));
  }
