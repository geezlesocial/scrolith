...
export const saveHeaderConfig = async (req: Request, res: Response) => {
  try {
    ...
    try {
      console.log('header.save navigation payload received:', safeLogLine(Array.isArray(config?.navigation) ? config.navigation.slice(0, 5) : config?.navigation));
    } catch (e) {
    ...
    try {
      ...
      console.log('saveHeaderConfig navigation sample:', safeLogLine(Array.isArray(config?.navigation) ? config.navigation.slice(0, 5) : config?.navigation));
    } catch (e) {
      /* ignore logging errors */
    }
