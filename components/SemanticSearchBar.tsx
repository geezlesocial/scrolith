...
import { SearchService } from "../src/services/search"; // adjust if your relative path differs
...
const SemanticSearchBar: React.FC<SemanticSearchBarProps> = ({ userId, className = "", placeholder }) => {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (userId) {
        await SearchService.saveSearchQuery(userId, query);
      }

      // Perform semantic search
      const data = await SearchService.getSemanticSearch(query);
      ...
      {(hasSearched || isSearching) && (
        <div className="mt-4 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden animate-fade-in-up">
          {isSearching ? (
            ...
            <div className="p-8 text-center text-gray-500">
              <p>No relevant results found matching your description.</p>
            </div>
          )}
        </div>
      )}
