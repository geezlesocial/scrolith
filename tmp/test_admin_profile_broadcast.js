...
(async () => {
  const serverScript = path.resolve(__dirname, '..', 'server', 'src', 'server.ts');
  console.log('Starting server as child process:', serverScript);

  const node = process.execPath;
  const child = spawn(node, [serverScript], { stdio: ['ignore', 'pipe', 'pipe'], env: { ...process.env, PORT: '5100' } });
